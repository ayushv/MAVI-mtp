...
def getRep(imgPath, multiple=False):
    ...
    if len(bbs) == 0 or (not multiple and bb1 is None):
        ...
        print("No face detected: {} skipping to next: ".format(imgPath))
        return []
    ...
    for img in args.imgs:
        ...
        if len(reps) >= 1:
            ...
            for r in reps:
                ...
                totalTime = totalTime + time.time() - startinfer
                print("Detection+Prediction took {} seconds.".format(time.time() - startinfer))
                if multiple:
                ...
                if img.find(person.decode('utf-8')) == -1:
                    print("Wrong prediction")
                else:
                    print("Correct prediction")
                    correct = correct + 1
    print("average time per image: {} sec".format(totalTime/totalChecked))
    print("Total Checked: {}".format(totalChecked))
    print("Total Correct: {}".format(correct))
    return float(correct)/totalChecked




def main(inputar):
	global args , net, align ,start
	sys.argv=inputar
	...
	if args.mode == 'train':
		...
		accuracy = infer(args, args.multi)
